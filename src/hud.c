...
void init_hud() BANKED
{
	...
	// Setup text box
	vmemset((char*) (0x9F80), HUD_TILE + 7u, 20);
	for (i = 0; i < 3; i++) {
		vmemset((char*) (0x9FA0 + i * 32), FONT_TILE - 1u, MESSAGE_SIZE / 3);
	}

	// Setup attack window
	vset(0x9C00, HUD_TILE + 9u);
	vmemset((char*) (0x9C01), HUD_TILE + 7u, 31);
	for (i = 1; i < 5; i++) {
...
void draw_move_window() NONBANKED
{
	...
	for (; i < 5; i++) {
		...
		vmemset((char*) (0x9C02 + i * 32), FONT_TILE - 1u, 8);
	}
...
{
	vmemcpy((char*) (0x9C21), 5, &arrow_window[0]);
	vmemcpy((char*) (0x9C41), 5, &arrow_window[5]);
	vmemcpy((char*) (0x9C61), 5, &arrow_window[10]);
	vmemset((char*) (0x9C81), FONT_TILE - 1u, 5);
}
...
void clear_print_hud() BANKED
{
	for (uint8_t i = 0; i < 3; i++)
		vmemset((char*) (0x9FA1 + i * 32), FONT_TILE - 1u,
			MESSAGE_SIZE / 3);
}
...
void draw_clock() BANKED
{
	...
	vwf_activate_font(0);
	vmemset((char*) (0x800Du + CLOCK_TILE * 16u), 0xFF, 3);
	vmemset((char*) (0x801Du + CLOCK_TILE * 16u), 0xFF, 3);
	vmemset((char*) (0x802Du + CLOCK_TILE * 16u), 0xFF, 3);
}
